<template>
  <DefaultLayout>
    <div class="flex justify-around p-[50px] h-screen w-full max-w-full bg-cover bg-no-repeat bg-[url('../public/Image/img_1.jpg')]">
...
<script setup>
  ...
  const handleLogin = async () => {
    try {
      const { data, error } = await useFetch('http://localhost:8080/v1/api/access/customer-site/login', {
        method: 'POST',
