<!-- Previous template code remains exactly the same until the editTable function in the script -->
...
<template>
    ...
    <Transition name="nested">
        <div v-if="addTableClicked" name="outer" class="absolute top-1/5 right-1/3">
            <hr>
            <form @submit.prevent
                ...
                <hr>
                <label class="font-bold underline text-lg text-[#3A6351]">Table ID: {{ tables.at(-1).id + 1 }}</label>
                ...
                <div class="flex justify-end gap-4">
                    ...
                    <button @click="handleCreateTable(tables.at(-1).id + 1)"
    ...
    <!-- Edit table form -->
    <Transition name="nested">
        <div v-if="editTableClicked" name="outer" class="absolute top-1/5 right-1/3">
            <hr>
            <form @submit.prevent
                class="w-full max-w-sm flex flex-col justify-center gap-2 rounded-lg shadow-[0px_0px_5px_green] bg-white p-4 px-11">
...
onMounted(() => {
    getData();
    console.log(JSON.stringify(tables.value.lastIndexOf))
})
...
const deleteSelectedTable = async () => {
    try {
        ...
        await getData() // Refresh the table list
    } catch (err) {
