<template>
    <div class="flex gap-10">
        <!-- chatbox -->
        <div class="flex-1 overflow-hidden relative" style="box-shadow: 0px 0px 128px rgba(0, 0, 0, 0.1);
        box-shadow: 0px 32px 64px -48px rgba(0, 0, 0, 0.5);">
            <!-- chat header -->
            <div class="bg-[#8CA899] py-4 pl-5">
                <h3 class="font-semibold text-lg text-white">{{ selectedCustomerName || 'Select a conversation' }}</h3>
            </div>
            <!-- chat body  -->
            <ul class="h-[500px] overflow-y-auto pt-7 px-5 pb-[70px]" ref="chatContainer">
                <li v-for="message in currentConversation" :key="message.id"
                    :class="{ 'flex my-1 gap-2': message.senderType === 'customer', 'flex my-1 justify-end': message.senderType === 'staff' }">
                    <span v-if="message.senderType === 'customer'"
                    ...
                    <p :class="{
                        ...
                        'text-black bg-white rounded-tl-lg rounded-tr-lg rounded-br-none rounded-bl-lg': message.senderType === 'customer',
                        'text-white bg-[#8CA899] rounded-tl-lg rounded-tr-lg rounded-br-none rounded-bl-lg': message.senderType === 'staff'
                    }">
                        {{ message.messageText }}
                    </p>
                </li>
            ...
            <!-- chat footer -->
            <div class="flex absolute bottom-0 bg-white w-full pt-1 px-5 border-t border-gray-300 gap-2 chat-input">
                ...
                <svg @click="sendMessage" xmlns="http://www.w3.org/2000/svg"
                    class="w-5 h-5 cursor-pointer self-center invisible" fill="#8CA899" viewBox="0 0 16 16">
                    <path
        ...
        <div class="w-full max-w-72 bg-white pt-4 px-4 h-fit">
            ...
            <div class="flex items-center gap-3 my-5 cursor-pointer hover:bg-gray-100 p-2 rounded"
                v-for="(table, index) in paginatedTables" :key="index" @click="selectConversation(table)">
            ...
            <!-- Pagination -->
            <div class="flex items-center mt-4 gap-2 w-full py-2 px-10 rounded bg-white">
...
const filteredTables = computed(() => {
    return tables.value.filter((table) => {
        return table.latestMessage.toLowerCase().includes(searchQuery.value.toLowerCase());
    });
});
...
const selectConversation = async (table) => {
    ...
    try {
        const { data: conversationData } = await useFetch(`http://localhost:8080/v1/api/message/get-conversation/${table.conversationID}`);
        currentConversation.value = conversationData.value.metadata;
        scrollToBottom();
    } catch (error) {
...
const sendMessage = async () => {
    ...
    try {
        const response = await fetch('http://localhost:8080/v1/api/message/send-message', {
            method: 'POST',
            headers: {
...
const getOpenConversation = async () => {
    try {
        const { data: conversationData } = await useFetch('http://localhost:8080/v1/api/message/get-open-conversations')
        tables.value = conversationData.value.metadata.map((item) => ({
        ...
        const customerRequests = tableIDs.map(async (customerID) => {
            try {
                const { data: customerData } = await useFetch(`http://localhost:8080/v1/api/customer-manage/customer/${customerID}`);
                customerNames[customerID] = customerData.value.metadata.name;
            }
            catch (e) {
