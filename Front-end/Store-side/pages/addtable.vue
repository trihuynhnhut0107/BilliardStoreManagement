<template>
  <div class="flex flex-col px-4 w-full h-full gap-4">
    <div
      class="flex justify-between items-center bg-white w-full py-2 px-10 rounded"
      style="box-shadow: 0px 0px 3px #a4a4a4">
      <h1 class="font-bold text-xl">Add New Table</h1>
    </div>
    <form
      @submit.prevent
      class="bg-white rounded-lg h-fit w-full px-10 py-3 flex justify-between"
      style="box-shadow: 0px 0px 3px #a4a4a4">
      <div class="flex flex-col gap-2 min-w-[350px]">
        <div class="flex items-center gap-2">
          <label
            for="tableType"
            class="text-base font-semibold w-full text-[#3A6351]"
            >Table type:
          </label>
          <input
            id="tableType"
        ...
        <div class="flex items-center gap-2">
          ...
          <input
            id="stickQuantity"
            v-model.number="formData.stick_quantity"
            type="number"
            required
        ...
        <div class="flex items-center gap-2">
          ...
          <input
            id="ballQuantity"
            v-model.number="formData.ball_quantity"
            type="number"
            required
...
const validateForm = () => {
  ...
  if (
    formData.value.stick_quantity <= 0 ||
    formData.value.ball_quantity <= 0 ||
