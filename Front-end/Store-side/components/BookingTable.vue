<template>
  <div class="w-full border-collapse border-none">
      ...
      <tbody>
        <template v-for="(item, index) in filteredDataList" :key="item.id">
          ...
          <tr
            ...
            :class="{
              'bg-blue-50 border-blue-300 w-[80px]':
            ...
            <td
              ...
              :key="key"
              class="p-2 text-center align-middle w-[80px]">
              {{
                isNaN(value[1])
                  ? toUpperCase(value[1])
            ...
            <td class="p-2 w-[80px] text-center">
              <button
                ...
                "
                @click="bookingCheckout(item.id)"
                class="py-[6px] px-4 bg-blue-500 text-white rounded"
                :class="`${
                  item.status == 'completed' || item.status == 'cancelled'
    ...
    <div
      ...
      <button
        @click="props.handlePageChange('prev')"
      ...
      >
      <button
        @click="props.handlePageChange('next')"
        :class="{
          'opacity-50 cursor-not-allowed':
...
const emit = defineEmits(["bookingCheckout"]);
