...
const baseURL = "http://localhost:3000"; // Update the base URL as needed

fixture`User Login Page Test`.page`${baseURL}/userlogin`; // Update the URL as needed
...
test("Verify page elements", async (t) => {
  ...
  await t
    ...
    .ok("Username input is present")
    .expect(passwordInput.exists)
    .ok("Password input is present")
    .expect(loginButton.exists)
    .ok("Login button is present")
    .expect(forgotPasswordLink.exists)
    .ok("Forgot password link is present")
    .expect(createAccountLink.exists)
    .ok("Create new account link is present");
});
...
test("Successful login", async (t) => {
  ...
  const passwordInput = Selector('input[placeholder="Password"]');
  const loginButton = Selector("button").withText("Log in");
  const dashboardTitle = Selector("h1").withText(
    "Your Ultimate Billiards Table Booking Hub"
  );

  // Step 1: Enter login credentials and click the login button
  await t
    .typeText(usernameInput, "testadmin") // Replace with valid test credentials
    .typeText(passwordInput, "testadmin")
    .click(loginButton);
  ...
  await t
    .setNativeDialogHandler((type, text) => {
      ...
      if (type === "alert") {
        ...
        return (
          text.includes("Login failed.") || text.includes("Please try again.")
        );
      }
    ...
    .ok("The page loaded after the login attempt.");
});
...
test("Navigation links", async (t) => {
  ...
  await t
    .navigateTo(`${baseURL}/userlogin`) // Navigate back to login page
