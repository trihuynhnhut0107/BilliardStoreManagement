...
fixture`Staff Management Page Test`.page`http://localhost:3000/login`;
...
test("Add staff with invalidate value", async (t) => {
  ...
  // Toast message
  const toastMessage = Selector(".Toastify__toast").withText(
...
test("Add staff with invalidate value", async (t) => {
  ...
  // Toast message
  const toastMessage =
    Selector(".Toastify__toast").withText("Name is required");
...
test("Edit Staff", async (t) => {
  ...
  await t
    ...
    .eql(
      "/staffmanagement",
      "Staff is not redirected to Customer Management page"
    );
  ...
  const editButton = customerRow
    .find("td.p-2 > svg")
    .withAttribute("fill", "#000");

  // Step 5: Click the Edit button
  await t
    .expect(editButton.exists)
    .ok("Edit button does not exist for the specified customer")
    .click(editButton);
